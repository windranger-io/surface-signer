...
export default function App() {
  ...
  const revokeSession = async (session: string) =>
    new Promise((resolve) => {
      // save details to background
      chrome.runtime.sendMessage(
        {
  ...
  useEffect(() => {
    chrome.runtime.onMessage.addListener(
      (request: {
        type: string;
        detail: { account?: string; signature?: string; loading?: boolean };
      }) => {
        switch (request.type) {
  ...
  return broken || accounts.length === 0 ? (
    <span />
  ) : (
    <>
      <style type="text/css">{styles}</style>
      <div className="mm-signer-wrapper">
        <div className={`mm-signer${isOpen ? ` open` : ``}`}>
          <div
            style={{
              width: "48px",
              height: "48px",
          ...
          {isOpen ? (
            <>
              <h1 className="tw-text-xl tw-text-slate-800 tw-my-2 tw-truncate">
                Delegated Signing Responsibilities on {document.location.host}
              </h1>
              <div className="tw-relative tw-rounded-xl tw-overflow-auto">
                <div className="tw-shadow-sm tw-my-8 tw-h-[296px] tw-overflow-auto">
                  <table className="tw-border-collapse tw-table-auto tw-w-full tw-text-sm">
                    <thead
                      className="tw-sticky tw-top-0 tw-bg-[#f8f7ce]"
                      style={{ boxShadow: "1px 0 0 #000000" }}
                    >
                      <tr>
                        <th className="tw-border-b tw-border-slate-600 tw-font-medium tw-p-4 tw-pl-8 tw-pt-0 tw-pb-3 tw-text-slate-800 tw-text-left">
                          Delegator
                        </th>
                        <th className="tw-border-b tw-border-slate-600 tw-font-medium tw-p-4 tw-pt-0 tw-pb-3 tw-text-slate-800 tw-text-left">
                          Signer
                        </th>
                        <th className="tw-border-b tw-border-slate-600 tw-font-medium tw-p-4 tw-pr-8 tw-pt-0 tw-pb-3 tw-text-slate-800 tw-text-left">
                          Msg Count
                        </th>
                        <th className="tw-border-b tw-border-slate-600 tw-font-medium tw-p-4 tw-pr-8 tw-pt-0 tw-pb-3 tw-text-slate-800 tw-text-left">
                          Status
                    ...
                    <tbody>
                      {Object.keys(history)
                        .sort(
                          (a: string, b: string) =>
                            new Date(history[b][0].issuedAt).getTime() -
                            new Date(history[a][0].issuedAt).getTime()
                        )
                        .map((session) => {
                          const { delegator, signer, expirationTime, revoked } =
                            history[session][0];
                          const total = history[session].length;
                          return (
                            <tr key={session}>
                              <td className="tw-border-b tw-border-slate-500 tw-p-4 tw-pl-8 tw-text-slate-800">
                                {delegator}
                              </td>
                              <td className="tw-border-b tw-border-slate-500 tw-p-4 tw-text-slate-800">
                                {signer}
                              </td>
                              <td className="tw-border-b tw-border-slate-500 tw-p-4 tw-pr-8 tw-text-slate-800">
                                {total}
                              </td>
                              <td className="tw-border-b tw-border-slate-500 tw-p-4 tw-pr-8 tw-text-slate-800">
                                {
